...
4. Confirm package metadata in `Cargo.toml` (`version`, `description`, `license`, `readme`,
   `documentation`, `repository`). README deep links must be absolute GitHub URLs so
   crates.io readers can open operator guides. Crate rustdoc (`src/lib.rs`) must state
   package name `blackbox-recorder` vs binary/library path `blackbox`.
