...
| **`daily_driver_notes`** | Human reasons (tips, penalties, crypto, eval, …) |
...
Missing surfaces still count against the denominator (as unknown) so the score stays honest.

Implementation: `CaptureCoverage::compute_quality_score` in `src/capture/coverage.rs`. Unit tests pin the weights.
