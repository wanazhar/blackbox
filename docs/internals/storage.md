...
The database carries a `schema_version` (currently **v10**). On open, the store checks the version and runs migrations sequentially. Migrations are wrapped in transactions — a failed migration rolls back cleanly.
...
| v10 | External evidence, edges, trace identity, provenance, findings, incidents |
